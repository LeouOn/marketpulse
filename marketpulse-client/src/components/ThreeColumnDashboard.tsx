...
export function ThreeColumnDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const failures = [dashboardResult, macroResult, breadthResult].filter(r => r.status === 'rejected');
      if (failures.length > 0) {
        setError(`Failed to load some market data: ${(failures[0] as PromiseRejectedResult).reason?.message || 'Unknown error'}`);
      }
  ...
  return (
    <div className="min-h-screen bg-black p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-3 space-y-4">
          {/* AI Assistant */}
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-4 flex flex-col" style={{ height: 'calc(100vh - 200px)', minHeight: '700px' }}>
            <div className="flex items-center gap-2 mb-4">
            ...
            <div className="flex-1 min-h-0">
              <LLMChat
                marketData={{
                  ...dashboardData?.data,
                  symbols: dashboardData?.data?.symbols || {},
