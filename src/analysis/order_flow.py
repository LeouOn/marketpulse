...
class ImbalanceDetector:
    ...
    def detect_imbalances(
        ...
        Args:
            volume_bars: List of VolumeBar objects
            candles: DataFrame with OHLC data (must have 'close' column and datetime index)
            lookback: Number of bars to analyze
        ...
        for i in range(len(volume_bars) - lookback + 1):
            ...
            price = self._resolve_price(candles, window[-1].timestamp)
    ...
    @staticmethod
    def _resolve_price(candles: Optional[pd.DataFrame], timestamp: datetime) -> float:
        if candles is None or candles.empty:
            return 0.0
        if timestamp in candles.index:
            return float(candles.loc[timestamp, 'close'])
        nearest_idx = candles.index.get_indexer([timestamp], method='nearest')[0]
        if nearest_idx >= 0:
            return float(candles.iloc[nearest_idx]['close'])
        return 0.0
