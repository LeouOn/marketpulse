...
class ICTAnalyzer:
    """Facade that wraps all ICT detectors with a unified interface"""

    def __init__(self, min_gap_size: float = 2.0, min_displacement: float = 5.0, swing_lookback: int = 10):
        self.fvg_detector = FairValueGapDetector(min_gap_size=min_gap_size)
        self.ob_identifier = OrderBlockIdentifier(min_displacement=min_displacement)
        self.liquidity_detector = LiquidityDetector()
        self.structure_analyzer = MarketStructureAnalyzer(swing_lookback=swing_lookback)

    def detect_fair_value_gaps(self, candles: pd.DataFrame) -> Dict[str, List[FairValueGap]]:
    ...
    def detect_order_blocks(self, candles: pd.DataFrame) -> List[OrderBlock]:
        return self.ob_identifier.identify_order_blocks(candles)

    def detect_liquidity_pools(self, candles: pd.DataFrame, lookback: int = 50) -> List[LiquidityPool]:
        return self.liquidity_detector.detect_liquidity_pools(candles, lookback=lookback)

    def analyze_market_structure(self, candles: pd.DataFrame) -> MarketStructure:
        swing_highs, swing_lows = self.structure_analyzer.identify_swing_points(candles)
        return self.structure_analyzer.determine_structure(swing_highs, swing_lows)
    ...
    def analyze(self, candles: pd.DataFrame) -> Dict[str, Any]:
        ...
        liquidity = self.detect_liquidity_pools(candles)
        structure = self.analyze_market_structure(candles)
        bos = self.structure_analyzer.detect_break_of_structure(candles, structure)
        return {
            'fvgs': fvgs,
