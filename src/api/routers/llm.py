...
async def get_available_models():
    ...
    try:
        ...
        ds_configured = bool(
            ds.api_key and ds.api_key not in ("your_deepseek_api_key", "")
        )
        models.append({
            "id": ds.model_pro,
            "provider": "deepseek",
        ...
        try:
            ...
            async with (
                ...
                if response.status == 200:
                    ...
                    status_info["providers"]["lm_studio"]["loaded_models"] = [
                        m["id"] for m in md.get("data", [])
                    ]
        except Exception as e:
            logger.debug(f"LM Studio status probe: {e}")

        return MarketResponse(
            success=True, data=status_info, timestamp=datetime.now().isoformat()
        )
...
import hashlib, pathlib, uuid as _uuid
...
async def submit_feedback(request: dict):
    try:
        ...
        feedback = {'analysis_id': analysis_id, 'rating': rating, 'outcome': outcome, 'notes': notes, 'timestamp': datetime.now().isoformat()}
        ...
        return MarketResponse(success=True, data={'feedback_id': fpath.stem, 'stored': True}, timestamp=datetime.now().isoformat())
    except Exception as e:
    ...
    try:
        ...
        for fpath in _feedback_dir.glob('*.json'):
            try:
                ...
                pass
        if not feedbacks:
            return MarketResponse(success=True, data={'total': 0, 'message': 'No feedback yet'}, timestamp=datetime.now().isoformat())
        ratings = [f['rating'] for f in feedbacks if 'rating' in f]
        outcomes = {}
        for f in feedbacks:
            o = f.get('outcome', 'unknown')
            outcomes[o] = outcomes.get(o, 0) + 1
        return MarketResponse(success=True, data={
            'total': len(feedbacks),
