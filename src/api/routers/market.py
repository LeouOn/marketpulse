...
from .deps import MarketPulseCollector, MarketResponse, settings
    ...
    try:
        ...
        try:
            ...
            for indicator in ["DXY", "TNX", "GC", "BTC"]:
                if (
                    indicator in macro_data
                    and isinstance(macro_data[indicator], dict)
                    and "price" in macro_data[indicator]
                ):
...
async def get_ohlc_analysis(symbol: str):
    ...
    try:
        ...
        ohlc_analyzer = _deps.ohlc_analyzer
        historical_data = {}
        client = YahooFinanceClient(settings)
