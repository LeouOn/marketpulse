...
# ---------------------------------------------------------------------------
# Function-calling tool definition for structured hypothesis output
# ---------------------------------------------------------------------------

HYPOTHESIS_RESULT_TOOL = {
    "type": "function",
    "function": {
        "name": "submit_hypothesis_result",
        "description": "Submit the structured result of a hypothesis test. Call this with your complete analysis.",
        "parameters": {
            "type": "object",
            "properties": {
                "status": {
                    ...
                    "description": "The conclusion of the hypothesis test.",
                },
                "confidence": {
                    "type": "number",
                    "minimum": 0,
                    "maximum": 100,
                    "description": "Confidence level (0-100) in the conclusion.",
                },
                "summary": {
                    "type": "string",
                    "description": "One-paragraph summary of the test result.",
                },
                "key_findings": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Bullet-point key findings from the analysis.",
                },
                "statistical_evidence": {
                    "type": "object",
                    "description": "Key statistical metrics (p-value, effect size, sample size, etc.).",
                    "additionalProperties": True,
                },
                "trading_implications": {
                    "type": "string",
                    "description": "What this result means for trading -- entries, exits, risk adjustments.",
                },
                "further_testing_needed": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "What additional testing would strengthen or refute the conclusion.",
                },
            },
            "required": [
                "status",
...
# ---------------------------------------------------------------------------
# HypothesisTestResult dataclass
# ---------------------------------------------------------------------------
...
class HypothesisTester:
    ...
    def __init__(self, llm_client, knowledge_rag: TradingKnowledgeRAG | None = None):
    ...
    def load_hypothesis(self, hypothesis_name: str) -> dict[str, Any] | None:
        """
    ...
    def _parse_data_requirements(self, content: str) -> dict[str, Any]:
        ...
        requirements = {"instruments": [], "timeframe": "", "features": [], "control": ""}
    ...
    async def clarify_hypothesis(self, hypothesis: dict[str, Any]) -> str:
        ...
        # Build prompt
        prompt = build_enhanced_prompt(HYPOTHESIS_TESTING_PROMPT, context_chunks, hypothesis["description"])

        # Query LLM
        messages = [{"role": "user", "content": prompt}]

        response = await self.llm_client.generate_completion(
            model="deep_analysis", messages=messages, max_tokens=500, temperature=0.3
        )
    ...
    async def analyze_hypothesis_with_data(self, hypothesis: dict[str, Any], market_data: dict[str, Any]) -> str:
        ...
        prompt = build_enhanced_prompt(
            HYPOTHESIS_TESTING_PROMPT, context_chunks, hypothesis["description"], market_data
        )
        ...
        response = await self.llm_client.generate_completion(
            model="deep_analysis", messages=messages, max_tokens=600, temperature=0.3
        )
    ...
    async def test_hypothesis(
        ...
        # Try structured output (function calling) first;
        # fall back to regex parsing if the client doesn't support it.
        result = await self._parse_with_structured_output(
            hypothesis_name, hypothesis, market_data
        )
        if result is None:
            # Fallback: traditional text analysis → regex parse
    ...
    async def _parse_with_structured_output(
        ...
        """Attempt to get structured output via function calling.

        Only works when ``self.llm_client`` supports
        ``generate_with_tools`` (DeepSeekClient, ModelRouter).

        Returns a typed ``HypothesisTestResult`` on success, or
        ``None`` if the client doesn't support function calling or
        the call fails.
        """
        if not hasattr(self.llm_client, "generate_with_tools"):
            return None

        # Build context and prompt
        context_chunks = self.knowledge_rag.retrieve_context(
            hypothesis.get("description", hypothesis_name)
        )
        prompt = build_enhanced_prompt(
        ...
        # Handler for the function-calling loop -- the model calls
        # ``submit_hypothesis_result(...)`` and we catch the args.
        captured: dict[str, Any] | None = None

        async def tool_handler(name: str, args: dict) -> dict:
            nonlocal captured
            if name == "submit_hypothesis_result":
                captured = args
        ...
        try:
            ...
            logger.warning(f"Structured hypothesis parse failed: {e}")
            return None

        if captured is None:
            # Model didn't call the tool -- fall back to regex
            if response and "choices" in response:
                raw = response["choices"][0]["message"].get("content", "")
                if raw:
                    return self._parse_analysis_results(
                        hypothesis_name, raw, market_data
                    )
            return None
        ...
        return HypothesisTestResult(
            hypothesis_name=hypothesis_name,
            timestamp=datetime.now().isoformat(),
            status=captured.get("status", "inconclusive"),
            confidence=float(captured.get("confidence", 50)),
            summary=captured.get("summary", "Analysis completed"),
            key_findings=captured.get("key_findings", []),
            statistical_evidence=captured.get("statistical_evidence", {}),
            trading_implications=captured.get("trading_implications", ""),
            further_testing_needed=captured.get("further_testing_needed", []),
            raw_analysis=json.dumps(captured, indent=2),
        )

    def _parse_analysis_results(
        self, hypothesis_name: str, analysis: str, market_data: dict[str, Any]
    ) -> HypothesisTestResult:
        """Parse LLM analysis into structured results (regex fallback)."""
        # Default values
        status = "inconclusive"
        confidence = 50
    ...
    def list_hypotheses(self) -> list[dict[str, str]]:
        ...
        if active_dir.exists():
            for md_file in active_dir.glob("*.md"):
                hypotheses.append({"name": md_file.stem, "status": "active", "file": str(md_file)})

        # Check tested hypotheses
        tested_dir = self.hypotheses_dir / "tested"
        if tested_dir.exists():
            for md_file in tested_dir.glob("*.md"):
                hypotheses.append({"name": md_file.stem, "status": "tested", "file": str(md_file)})
    ...
    async with LMStudioClient() as client:
        ...
        if any(h["name"] == "overnight_margin_cascade" for h in hypotheses):
            ...
            sample_data = {
                ...
                "time_window": {"start": "23:45:00", "end": "00:15:00", "price_change": -0.8, "volume_spike": 2.3},
            }
            ...
            # Save results
            results_file = Path(
                f"hypothesis_results_{result.hypothesis_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
