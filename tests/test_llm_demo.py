...
async def demo_basic_query():
    ...
    async with LMStudioClient() as client:
        ...
        result = await client.generate_completion(
            model="fast_analysis", messages=messages, max_tokens=10, temperature=0.1
        )
...
async def demo_chart_analysis():
    ...
    chart_data = {
        ...
        "indicators": {"sma_20": 15025, "rsi": 72.3, "volume_ma": 1300},
    }
...
async def demo_roundtrip_conversation():
    ...
    async with LMStudioClient() as client:
        ...
        messages = [
            {
                "role": "user",
                "content": "What are the key factors for successful day trading NQ futures? List 3 main points.",
            }
        ]

        print("\nQ1: What are key factors for day trading NQ futures?")
        result1 = await client.generate_completion(
            model="fast_analysis", messages=messages, max_tokens=150, temperature=0.3
        )
        ...
        if result1 and "choices" in result1:
            ...
            result2 = await client.generate_completion(
                model="deep_analysis", messages=messages, max_tokens=200, temperature=0.4
            )
...
async def main():
    ...
    print("=" * 70)
    print("LM Studio: http://127.0.0.1:1234/v1")
    print("Model: aquif-3.5-max-42b-a3b-i1")
    print("=" * 70 + "\n")
