...
async def test_lm_studio_connection():
    ...
    try:
        ...
        async with LMStudioClient(settings) as client:
            ...
            messages = [{"role": "user", "content": "What is 15 + 27? Respond with just the number."}]

            result = await client.generate_completion(
                model="fast_analysis", messages=messages, max_tokens=5, temperature=0.1
            )
...
async def test_text_chart_interpretation():
    ...
    try:
        ...
        chart_data = {
            ...
            "indicators": {"sma_20": 15010, "rsi": 65.5, "volume_ma": 1100},
        }
...
async def run_comprehensive_llm_test():
    ...
    print("=" * 80)
    print("LM Studio Endpoint: http://127.0.0.1:1234/v1")
    print("Test Model: aquif-3.5-max-42b-a3b-i1")
